...
import com.niche.jhipster.service.SectorService;
...
// import com.niche.jhipster.service.SectorRepository;
...
public class SectorResource {
    ...
    /**
     * Load particular zone sector list value details from table 
     * using Interface (SectorRepository)
     * 
     * @return Array of sector details
     */
    /* @GetMapping("/get-zone-sectors/{id}")
    @Timed
    public ResponseEntity<List<Object[]>> getZonalSectors(@PathVariable Long id) {
        log.debug("REST request to get all Sector from particular Zone : {}", id);
        List<Object[]> sectorList = sectorRepository.findSectors(id);
        System.out.println("sectorList : " + sectorList);
        return ResponseUtil.wrapOrNotFound(sectorList);
    } */
}
